...
const submit = btn.addEventListener("click", ()=>{
    rgb1 = [hexToRgb(color1.value).r, hexToRgb(color1.value).g, hexToRgb(color1.value).b]
    rgb2 = [hexToRgb(color2.value).r, hexToRgb(color2.value).g, hexToRgb(color2.value).b]
    contrast();
...
const showContrastMessage = () => {
    ...
    if (ratio >= 7) {
        ...
        message.textContent = "The colors passed, but may not work well with blocks of small text!!";
        message.style.color = "#333333";
    } else {
